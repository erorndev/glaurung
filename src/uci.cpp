...
std::string UCIInputParser::get_next_token() {
  ...
  for (i = j = this->currentIndex;
       j < this->length && !isspace(this->inputLine[j]); j++)
    ;
  this->currentIndex = j;
  this->skip_whitespace();
...
void handle_command(const std::string &command) {
  ...
  // size of the program binary.
    ...
    EvalInfo ei;
    std::cout << "Incremental mg: " << RootPosition.mg_value() << std::endl;
    std::cout << "Incremental eg: " << RootPosition.eg_value() << std::endl;
    std::cout << "Full eval: " << evaluate(RootPosition, ei, 0) << std::endl;
  } else if (s == "key") {
...
void set_position(UCIInputParser &uip) {
  ...
  if (!uip.at_end_of_line()) {
    if (token != "moves") token = uip.get_next_token();
    if (token == "moves") {
      Move move;
      UndoInfo u;
      while (!uip.at_end_of_line()) {
        ...
        if (RootPosition.rule_50_counter() == 0) RootPosition.reset_game_ply();
      }
    }
  }
}
...
void go(UCIInputParser &uip) {
  ...
  while (!uip.at_end_of_line()) {
    ...
    if (token == "infinite")
      ...
      if (!uip.at_end_of_line()) time[0] = atoi(uip.get_next_token().c_str());
    } else if (token == "btime") {
      if (!uip.at_end_of_line()) time[1] = atoi(uip.get_next_token().c_str());
    } else if (token == "winc") {
      if (!uip.at_end_of_line()) inc[0] = atoi(uip.get_next_token().c_str());
    } else if (token == "binc") {
      if (!uip.at_end_of_line()) inc[1] = atoi(uip.get_next_token().c_str());
    } else if (token == "movestogo") {
      if (!uip.at_end_of_line()) movesToGo = atoi(uip.get_next_token().c_str());
    } else if (token == "depth") {
      if (!uip.at_end_of_line()) depth = atoi(uip.get_next_token().c_str());
    } else if (token == "nodes") {
      if (!uip.at_end_of_line()) nodes = atoi(uip.get_next_token().c_str());
    } else if (token == "movetime") {
      if (!uip.at_end_of_line()) moveTime = atoi(uip.get_next_token().c_str());
  ...
  if (moveTime) infinite = true; // HACK
