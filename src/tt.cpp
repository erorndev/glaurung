...
void TranspositionTable::store(const Position &pos, Value v, Depth d, Move m,
                               ValueType type) {
  TTEntry *tte, *replace;
  Key key = pos.get_key();

  tte = replace = entries + int(key & (size - 1)) * 4;

  _mm_prefetch((const char *)tte, _MM_HINT_T0);

  for (int i = 0; i < 4; i++) {
    if ((tte + i)->key() == key) {
      if (m == MOVE_NONE) m = (tte + i)->move();
      *(tte + i) = TTEntry(key, v, type, d, m, generation);
      return;
    }
  ...
  *replace = TTEntry(key, v, type, d, m, generation);
  writes++;
}
...
bool TranspositionTable::retrieve(const Position &pos, Value *value, Depth *d,
                                  Move *move, ValueType *type) const {
  TTEntry *tte;
  bool found = false;
  Key key = pos.get_key();

  tte = entries + int(key & (size - 1)) * 4;
...
TTEntry::TTEntry(Key k, Value v, ValueType t, Depth d, Move m, int generation) {
  key_ = k;
