...
const Value MidgameKnightMobilityBonus[] = {Value(-30), Value(-20), Value(-10),
                                            Value(0),   Value(10),  Value(20),
                                            Value(25),  Value(30),  Value(30)};

const Value EndgameKnightMobilityBonus[] = {Value(-30), Value(-20), Value(-10),
                                            Value(0),   Value(10),  Value(20),
                                            Value(25),  Value(30),  Value(30)};
...
const Value MidgameQueenMobilityBonus[] = {
    Value(-10), Value(-8), Value(-6), Value(-4), Value(-2), Value(0),
    Value(2),   Value(4),  Value(6),  Value(8),  Value(10), Value(12),
    Value(13),  Value(14), Value(15), Value(16), Value(16), Value(16),
...
const Value KnightOutpostBonus[64] = {
    ...
    Value(0),  Value(0),  Value(0),  Value(0),  Value(0), Value(0),  Value(0),
    Value(0),  Value(0),  Value(0),  Value(0),  Value(5), Value(10), Value(10),
    Value(5),  Value(0),  Value(0),  Value(0),  Value(5), Value(20), Value(30),
    Value(30), Value(20), Value(5),  Value(0),  Value(0), Value(10), Value(30),
    Value(40), Value(40), Value(30), Value(10), Value(0), Value(0),  Value(5),
    Value(20), Value(20), Value(20), Value(20), Value(5), Value(0),  Value(0),
    Value(0),  Value(0),  Value(0),  Value(0),  Value(0), Value(0),  Value(0),
    Value(0),  Value(0),  Value(0),  Value(0),  Value(0), Value(0),  Value(0),
    Value(0)};
...
const Value BishopOutpostBonus[64] = {
    Value(0),  Value(0),  Value(0),  Value(0),  Value(0), Value(0),  Value(0),
    Value(0),  Value(0),  Value(0),  Value(0),  Value(0), Value(0),  Value(0),
    Value(0),  Value(0),  Value(0),  Value(0),  Value(5), Value(5),  Value(5),
    Value(5),  Value(0),  Value(0),  Value(0),  Value(5), Value(10), Value(10),
    Value(10), Value(10), Value(5),  Value(0),  Value(0), Value(10), Value(20),
    Value(20), Value(20), Value(20), Value(10), Value(0), Value(0),  Value(5),
    Value(8),  Value(8),  Value(8),  Value(8),  Value(5), Value(0),  Value(0),
    Value(0),  Value(0),  Value(0),  Value(0),  Value(0), Value(0),  Value(0),
    Value(0),  Value(0),  Value(0),  Value(0),  Value(0), Value(0),  Value(0),
    Value(0)};
...
const Bitboard MaskA7H7[2] = {((1ULL << SQ_A7) | (1ULL << SQ_H7)),
...
const Bitboard MaskA1H1[2] = {((1ULL << SQ_A1) | (1ULL << SQ_H1)),
...
const int InitKingDanger[64] = {
    2,  0,  2,  5,  5,  2,  0,  2,  2,  2,  4,  8,  8,  4,  2,  2,  7,  10, 12,
    12, 12, 12, 10, 7,  15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
...
Value evaluate(const Position &pos, EvalInfo &ei, int threadID) {
  ...
  ei.attackedBy[WHITE][PAWN] = ((pos.pawns(WHITE) << 9) & ~FileABB) |
                               ((pos.pawns(WHITE) << 7) & ~FileHBB);
  ei.attackCount[WHITE] +=
      count_1s_max_15(ei.attackedBy[WHITE][PAWN] & ei.attackedBy[BLACK][KING]) /
      2;
  ei.attackedBy[BLACK][PAWN] = ((pos.pawns(BLACK) >> 7) & ~FileABB) |
                               ((pos.pawns(BLACK) >> 9) & ~FileHBB);
  ei.attackCount[BLACK] +=
      count_1s_max_15(ei.attackedBy[BLACK][PAWN] & ei.attackedBy[WHITE][KING]) /
      2;
  ...
  if (ei.pi->passed_pawns()) evaluate_passed_pawns(pos, ei);
  ...
  if (phase < PHASE_MIDGAME && pos.opposite_colored_bishops() &&
    ...
    if (pos.non_pawn_material(WHITE) + pos.non_pawn_material(BLACK) ==
      ...
      // bishops.
      if (factor[WHITE] == SCALE_FACTOR_NORMAL) factor[WHITE] = ScaleFactor(50);
      if (factor[BLACK] == SCALE_FACTOR_NORMAL) factor[BLACK] = ScaleFactor(50);
    }
  }
...
void init_eval(int threads) {
  ...
  for (int i = 0; i < threads; i++) {
    if (PawnTable[i] == NULL) PawnTable[i] = new PawnInfoTable(PawnTableSize);
...
void read_weights(Color sideToMove) {
  ...
  WeightKingSafety[sideToMove] = compute_weight(
      get_option_value_int("Cowardice"), WeightKingSafetyInternal);
  WeightKingSafety[opposite_color(sideToMove)] = compute_weight(
      get_option_value_int("Aggressiveness"), WeightKingSafetyInternal);
  WeightSpace =
...
void evaluate_knight(const Position &p, Square s, Color us, EvalInfo &ei) {
  ...
  if (p.square_is_weak(s, them)) {
    ...
    if (v && p.pawn_attacks(them, s) & p.pawns(us)) {
      bonus += v / 2;
      if (p.knight_count(them) == 0 && (SquaresByColorBB[square_color(s)] &
...
void evaluate_king(const Position &p, Square s, Color us, EvalInfo &ei) {
  ...
  if (p.queen_count(them) >= 1 && ei.attackCount[them] >= 2 &&
    ...
    Bitboard undefended = ei.attacked_by(them) & ~ei.attacked_by(us, PAWN) &
                          ~ei.attacked_by(us, KNIGHT) &
                          ~ei.attacked_by(us, BISHOP) &
                          ~ei.attacked_by(us, ROOK) &
                          ~ei.attacked_by(us, QUEEN) & ei.attacked_by(us, KING);
    ...
    if (ei.mateThreat[them] != MOVE_NONE) attackUnits += MateThreatBonus;
    ...
    ei.mgValue -= sign * v;
    if (us == p.side_to_move()) ei.futilityMargin += v;
  }
}
  ...
  for (Color us = WHITE; us <= BLACK; us++) {
    ...
    while (b) {
      ...
      if (pos.square_is_empty(blockSq)) {
        ...
        if (bit_is_set(ei.attacked_by(them, ROOK) | ei.attacked_by(them, QUEEN),
                       s) &&
        ...
        // in the pawn's path:
        if ((b2 & pos.pieces_of_color(us)) == EmptyBoardBB) ebonus += Value(tr);
      }
      ...
      if (pos.non_pawn_material(them) == Value(0)) {
        ...
        if (d < 0) {
          ...
          int blockerCount = count_1s_max_15(squares_in_front_of(us, s) &
...
void evaluate_space(const Position &pos, Color us, EvalInfo &ei) {
  ...
  Bitboard safeSquares = SpaceMask[us] & ~pos.pawns(us) &
...
void init_safety() {
  ...
  for (i = 0; i < 100; i++)
    if (SafetyTable[i] > Value(peak)) SafetyTable[i] = Value(peak);
}
