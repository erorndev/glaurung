...
const uint8_t KBNKMateTable[64] = {
    200, 190, 180, 170, 160, 150, 140, 130, 190, 180, 170, 160, 150,
    140, 130, 140, 180, 170, 155, 140, 140, 125, 140, 150, 170, 160,
    140, 120, 110, 140, 150, 160, 160, 150, 140, 110, 120, 140, 160,
    170, 150, 140, 125, 140, 140, 155, 170, 180, 140, 130, 140, 150,
    160, 170, 180, 190, 130, 140, 150, 160, 170, 180, 190, 200};
...
KXKEvaluationFunction::KXKEvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
KBNKEvaluationFunction::KBNKEvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
KPKEvaluationFunction::KPKEvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
KRKPEvaluationFunction::KRKPEvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
KRKBEvaluationFunction::KRKBEvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
KRKNEvaluationFunction::KRKNEvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
KQKREvaluationFunction::KQKREvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
KBBKNEvaluationFunction::KBBKNEvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
KmmKmEvaluationFunction::KmmKmEvaluationFunction(Color c)
    : EndgameEvaluationFunction(c) {}
...
Value KRKPEvaluationFunction::apply(const Position &pos) {
  ...
  // the position is drawish:
    ...
    result = Value(200) - Value(square_distance(wksq, bpsq + DELTA_S) * 8) +
...
Value KRKNEvaluationFunction::apply(const Position &pos) {
  ...
  Value result =
      Value(10) + mate_table(defendingKSq) +
...
ScaleFactor KRPKRScalingFunction::apply(const Position &pos) {
  ...
  if (r <= RANK_5 && square_distance(bksq, queeningSq) <= 1 && wksq <= SQ_H5 &&
      (square_rank(brsq) == RANK_6 ||
       (r <= RANK_3 && square_rank(wrsq) != RANK_6)))
    return ScaleFactor(0);
  ...
  if (r <= RANK_4 && bksq > wpsq) {
    if (square_file(bksq) == square_file(wpsq)) return ScaleFactor(10);
...
ScaleFactor KBPKBScalingFunction::apply(const Position &pos) {
  ...
  if (square_color(strongerBishopSq) != square_color(weakerBishopSq)) {
    ...
    if (pawn_rank(strongerSide, pawnSq) <= RANK_5)
      ...
      if (ray & pos.kings(weakerSide)) return ScaleFactor(0);
