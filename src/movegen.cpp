...
int generate_knight_moves(const Position &pos, MoveStack *mlist, Color side,
                          Bitboard target);
int generate_bishop_moves(const Position &pos, MoveStack *mlist, Color side,
                          Bitboard target);
int generate_rook_moves(const Position &pos, MoveStack *mlist, Color side,
                        Bitboard target);
int generate_queen_moves(const Position &pos, MoveStack *mlist, Color side,
                         Bitboard target);
int generate_king_moves(const Position &pos, MoveStack *mlist, Square from,
                        Bitboard target);
...
int generate_evasions(const Position &pos, MoveStack *mlist) {
  ...
  if (!(checkers & (checkers - 1))) {
    ...
    b1 = pos.bishop_attacks(checksq) & pos.bishops_and_queens(us) & ~pinned;
    while (b1) {
      from = pop_1st_bit(&b1);
      mlist[n++].move = make_move(from, checksq);
    }
    ...
    b1 = pos.rook_attacks(checksq) & pos.rooks_and_queens(us) & ~pinned;
    while (b1) {
      from = pop_1st_bit(&b1);
      mlist[n++].move = make_move(from, checksq);
    }
...
Move generate_move_if_legal(const Position &pos, Move m, Bitboard pinned) {
  ...
  if (color_of_piece(pc) != us) return MOVE_NONE;
  ...
  if (move_is_ep(m)) {
    ...
    if (type_of_piece(pc) != PAWN) return MOVE_NONE;

    // The destination square must be the en passant square:
    if (to != pos.ep_square()) return MOVE_NONE;
  ...
  // Castling moves:
    ...
    if (type_of_piece(pc) != KING) return MOVE_NONE;

    // The side to move must still have the right to castle kingside:
    if (!pos.can_castle_kingside(us)) return MOVE_NONE;
    ...
    for (s = Min(to, f1); s <= Max(to, f1); s++)
      if (s != from && s != to && !pos.square_is_empty(s)) illegal = true;
    ...
    // The side to move must still have the right to castle kingside:
    if (!pos.can_castle_queenside(us)) return MOVE_NONE;
  ...
  // Normal moves
    ...
    if (pos.color_of_piece_on(to) == us) return MOVE_NONE;
    ...
    case PAWN:
      ...
      if (us == WHITE) {
        ...
        // promotion.
        if (square_rank(to) == RANK_8 && !move_promotion(m)) return MOVE_NONE;
        ...
        case DELTA_NE:
          ...
          if (pos.color_of_piece_on(to) != them) return MOVE_NONE;
          break;

        case DELTA_N:
          // Pawn push.  The destination square must be empty.
          if (!pos.square_is_empty(to)) return MOVE_NONE;
          break;
        ...
        // promotion.
        if (square_rank(to) == RANK_1 && !move_promotion(m)) return MOVE_NONE;
  ...
  if (pos.can_castle(us)) {
    ...
    if (pos.can_castle_kingside(us)) {
      ...
      for (s = Min(rsq, f1); s <= Max(rsq, f1); s++)
        if (s != ksq && s != rsq && pos.square_is_occupied(s)) illegal = true;

      if (!illegal) mlist[n++].move = make_castle_move(ksq, rsq);
    }
