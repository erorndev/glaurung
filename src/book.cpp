...
const int PieceTo12[] = {0, 0, 2, 4, 6, 8, 10, 0, 0, 1, 3, 5, 7, 9, 11};
...
Move Book::get_move(const Position &pos) const {
  if (this->is_open()) {
    ...
    for (int i = this->find_key(key); i < bookSize; i++) {
      ...
      bestScore += score;
      if (int(genrand_int32() % bestScore) < score) bestMove = move;
    }
    ...
    if (bestMove != 0) {
      ...
      for (j = 0; j < n; j++)
        if ((int(moves[j].move) & 07777) == bestMove) return moves[j].move;
    }
  }
  return MOVE_NONE;
}
...
uint64_t book_castle_key(const Position &pos) {
  ...
  if (pos.can_castle_kingside(WHITE)) result ^= Random64[RandomCastle + 0];
  if (pos.can_castle_queenside(WHITE)) result ^= Random64[RandomCastle + 1];
  if (pos.can_castle_kingside(BLACK)) result ^= Random64[RandomCastle + 2];
  if (pos.can_castle_queenside(BLACK)) result ^= Random64[RandomCastle + 3];
  return result;
}
...
uint64_t book_ep_key(const Position &pos) {
  return (pos.ep_square() == SQ_NONE)
             ? 0ULL
             : Random64[RandomEnPassant + square_file(pos.ep_square())];
}
