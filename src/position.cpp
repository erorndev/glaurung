...
void Position::from_fen(const std::string &fen) {
  ...
  for (i = 0; fen[i] != ' '; i++) {
    if (isdigit(fen[i]))
      ...
      switch (fen[i]) {
      case 'K':
        this->put_piece(WK, square);
        file++;
        break;
      case 'Q':
        this->put_piece(WQ, square);
        file++;
        break;
      case 'R':
        this->put_piece(WR, square);
        file++;
        break;
      case 'B':
        this->put_piece(WB, square);
        file++;
        break;
      case 'N':
        this->put_piece(WN, square);
        file++;
        break;
      case 'P':
        this->put_piece(WP, square);
        file++;
        break;
      case 'k':
        this->put_piece(BK, square);
        file++;
        break;
      case 'q':
        this->put_piece(BQ, square);
        file++;
        break;
      case 'r':
        this->put_piece(BR, square);
        file++;
        break;
      case 'b':
        this->put_piece(BB, square);
        file++;
        break;
      case 'n':
        this->put_piece(BN, square);
        file++;
        break;
      case 'p':
        this->put_piece(BP, square);
        file++;
        break;
      case '/':
        file = FILE_A;
  ...
  while (strchr("KQkqabcdefghABCDEFGH-", fen[i])) {
    if (fen[i] == '-') {
      ...
      for (Square square = SQ_B1; square <= SQ_G1; square++)
        if (this->piece_on(square) == WK) kingFile = square_file(square);
      ...
      for (Square square = SQ_B8; square <= SQ_G8; square++)
        if (this->piece_on(square) == BK) kingFile = square_file(square);
      if (kingFile == FILE_NONE) {
        std::cout << "Error in FEN at character " << i << std::endl;
        return;
      }
...
bool Position::square_is_attacked(Square s, Color c) const {
  return (this->pawn_attacks(opposite_color(c), s) & this->pawns(c)) ||
...
Bitboard Position::attacks_to(Square s) const {
  return (this->black_pawn_attacks(s) & this->pawns(WHITE)) |
...
bool Position::move_is_legal(Move m, Bitboard pinned) const {
  ...
  if (move_is_ep(m)) {
    ...
    clear_bit(&b, capsq);
    set_bit(&b, to);
    return (!(rook_attacks_bb(ksq, b) & this->rooks_and_queens(them)) &&
...
bool Position::move_is_check(Move m, Bitboard dcCandidates) const {
  ...
  case PAWN:
    // Normal check?
    if (bit_is_set(this->pawn_attacks(them, ksq), to)) return true;
    // Discovered check?
    ...
    // captured pawn.
      ...
      clear_bit(&b, capsq);
      set_bit(&b, to);
      return ((rook_attacks_bb(ksq, b) & this->rooks_and_queens(us)) ||
  ...
  case KNIGHT:
    // Discovered check?
    if (bit_is_set(dcCandidates, from)) return true;
    // Normal check?
    else
...
bool Position::move_is_capture(Move m) const {
  return this->color_of_piece_on(move_to(m)) ==
             opposite_color(this->side_to_move()) ||
         move_is_ep(m);
}
...
void Position::do_move(Move m, UndoInfo &u, Bitboard dcCandidates) {
  ...
  if (move_is_castle(m))
    ...
    if (capture) {
      ...
      if (capture == PAWN) pawnKey ^= zobrist[them][PAWN][to];
      ...
      // Update material:
      if (capture != PAWN) npMaterial[them] -= piece_value_midgame(capture);
    ...
    if (piece == KING) kingSquare[us] = to;
    ...
    if (piece == PAWN) {
      if (abs(int(to) - int(from)) == 16) {
        if ((us == WHITE &&
             (this->white_pawn_attacks(from + DELTA_N) & this->pawns(BLACK))) ||
            (us == BLACK &&
             (this->black_pawn_attacks(from + DELTA_S) & this->pawns(WHITE)))) {
    ...
    case KNIGHT:
      if (bit_is_set(this->knight_attacks(ksq), to)) set_bit(&checkersBB, to);
      if (bit_is_set(dcCandidates, from))
        checkersBB |=
            ((this->rook_attacks(ksq) & this->rooks_and_queens(us)) |
             (this->bishop_attacks(ksq) & this->bishops_and_queens(us)));
    ...
    case BISHOP:
      if (bit_is_set(this->bishop_attacks(ksq), to)) set_bit(&checkersBB, to);
      if (bit_is_set(dcCandidates, from))
        checkersBB |= (this->rook_attacks(ksq) & this->rooks_and_queens(us));
      break;

    case ROOK:
      if (bit_is_set(this->rook_attacks(ksq), to)) set_bit(&checkersBB, to);
      if (bit_is_set(dcCandidates, from))
        checkersBB |=
            (this->bishop_attacks(ksq) & this->bishops_and_queens(us));
      break;
    ...
    case QUEEN:
      if (bit_is_set(this->queen_attacks(ksq), to)) set_bit(&checkersBB, to);
      break;
...
void Position::undo_move(Move m, const UndoInfo &u) {
  ...
  if (move_is_castle(m))
    ...
    // If the moving piece was a king, update the king square:
    if (piece == KING) kingSquare[us] = from;
...
void Position::do_null_move(UndoInfo &u) {
  ...
  sideToMove = opposite_color(sideToMove);
  if (epSquare != SQ_NONE) key ^= zobEp[epSquare];
  epSquare = SQ_NONE;
  rule50++;
  gamePly++;
  key ^= zobSideToMove;
...
int Position::see(Square from, Square to) const {
  ...
  static const int seeValues[18] = {0, 1, 3, 3, 5,  10,  100, 0, 0,
                                    1, 3, 3, 5, 10, 100, 0,   0, 0};
  Color us, them;
  Piece piece, capture;
  ...
  clear_bit(&occ, from);
  attackers = (rook_attacks_bb(to, occ) & this->rooks_and_queens()) |
...
Key Position::compute_key() const {
  ...
  if (this->ep_square() != SQ_NONE) result ^= zobEp[this->ep_square()];
...
Value Position::compute_mg_value() const {
  ...
  result += (this->side_to_move() == WHITE) ? (TempoValueMidgame / 2)
                                            : -(TempoValueMidgame / 2);
  return result;
}
...
Value Position::compute_eg_value() const {
  ...
  result += (this->side_to_move() == WHITE) ? (TempoValueEndgame / 2)
                                            : -(TempoValueEndgame / 2);
  return result;
}
...
bool Position::is_draw() const {
  ...
  if (rule50 > 100 || (rule50 == 100 && !this->is_check())) return true;

  // Draw by repetition?
  for (int i = 2; i < Min(gamePly, rule50); i += 2)
    if (history[gamePly - i] == key) return true;
...
void Position::flipped_copy(const Position &pos) {
  ...
  // En passant square
  if (pos.epSquare != SQ_NONE) epSquare = flip_square(pos.epSquare);
...
bool Position::is_ok() const {
  ...
  // Side to move OK?
  if (!color_is_ok(this->side_to_move())) return false;

  // Are the king squares in the position correct?
  if (this->piece_on(this->king_square(WHITE)) != WK) return false;
  if (this->piece_on(this->king_square(BLACK)) != BK) return false;

  // Castle files OK?
  if (!file_is_ok(initialKRFile)) return false;
  if (!file_is_ok(initialQRFile)) return false;
  ...
  if (debugKingCount) {
    ...
    if (kingCount[0] != 1 || kingCount[1] != 1) return false;
  }
  ...
  if (debugKingCapture) {
    ...
    Square ksq = this->king_square(them);
    if (this->square_is_attacked(ksq, us)) return false;
  }

  // Is there more than 2 checkers?
  if (debugCheckerCount && count_1s(checkersBB) > 2) return false;
  ...
  // Hash key OK?
  if (debugKey && key != this->compute_key()) return false;

  // Pawn hash key OK?
  if (debugPawnKey && pawnKey != this->compute_pawn_key()) return false;
  ...
  if (debugIncrementalEval) {
    if (mgValue != this->compute_mg_value()) return false;
    if (egValue != this->compute_eg_value()) return false;
  }

  // Non-pawn material OK?
  if (debugNonPawnMaterial) {
    if (npMaterial[WHITE] != compute_non_pawn_material(WHITE)) return false;
    if (npMaterial[BLACK] != compute_non_pawn_material(BLACK)) return false;
  }
  ...
  if (debugPieceCounts)
    for (Color c = WHITE; c <= BLACK; c++)
      for (PieceType pt = PAWN; pt <= KING; pt++)
        if (pieceCount[c][pt] !=
            count_1s(this->pieces_of_color_and_type(c, pt)))
          return false;
      ...
      for (PieceType pt = PAWN; pt <= KING; pt++)
        for (int i = 0; i < pieceCount[c][pt]; i++) {
          if (this->piece_on(this->piece_list(c, pt, i)) !=
            ...
            return false;
          if (index[this->piece_list(c, pt, i)] != i) return false;
        }
  }
