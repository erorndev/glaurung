...
/// this has been improved for xbox.
...
#define XBOX_INLINE inline __attribute__((always_inline))
...
/// these has been improved for xbox.

XBOX_INLINE Bitboard set_mask_bb(Square s) {
  //  return 1ULL << s;
  return SetMaskBB[s];
}

XBOX_INLINE Bitboard clear_mask_bb(Square s) {
  //  return ~set_mask_bb(s);
  return ClearMaskBB[s];
}

XBOX_INLINE Bitboard bit_is_set(Bitboard b, Square s) {
  return b & set_mask_bb(s);
}

XBOX_INLINE void set_bit(Bitboard *b, Square s) {
  *b |= set_mask_bb(s);
}

XBOX_INLINE void clear_bit(Bitboard *b, Square s) {
  *b &= clear_mask_bb(s);
}
...
inline Bitboard file_attacks_bb(Square s, Bitboard blockers) {
  ...
  return FileAttacks[square_rank(s)][(b * 0xd6e8802041d0c441ULL) >> 58] &
         file_bb(s);
}
...
/// this has been improved for xbox.
/// since we dont have popcnt, which sucks ass we can try to improve the sw
/// based approach
...
inline int count_1s(Bitboard b) {
  ...
  unsigned int combined = v + w;
  return (combined + (combined << 8) + (combined << 16) + (combined << 24)) >>
         24;
}
