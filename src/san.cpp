...
const std::string move_to_san(Position &pos, Move m) {
  ...
  if (m == MOVE_NONE) {
    ...
    if (pos.move_is_capture(m)) str += "x";
  ...
  UndoInfo u;
  pos.do_move(m, u);
  if (pos.is_check()) str += pos.is_mate() ? "#" : "+";
  pos.undo_move(m, u);
...
Move move_from_san(Position &pos, const std::string &movestr) {
  ...
  if (movestr == "O-O-O") {
    ...
    while ((m = mp.get_next_move()) != MOVE_NONE)
      if (move_is_long_castle(m) && pos.move_is_legal(m)) return m;
    return MOVE_NONE;
  } else if (movestr == "O-O") {
    Move m;
    while ((m = mp.get_next_move()) != MOVE_NONE)
      if (move_is_short_castle(m) && pos.move_is_legal(m)) return m;
    return MOVE_NONE;
  }
  ...
  *cc = '\0';
  ...
  // Find the to square:
  if (left < right) {
    if (str[right] < '1' || str[right] > '8' || str[right - 1] < 'a' ||
        str[right - 1] > 'h')
      return MOVE_NONE;
    to =
        make_square(file_from_char(str[right - 1]), rank_from_char(str[right]));
    right -= 2;
  } else
    return MOVE_NONE;
  ...
  if (left <= right) {
    ...
    if (strchr("12345678", str[left])) fromRank = rank_from_char(str[left]);
  }
...
Ambiguity move_ambiguity(Position &pos, Move m) {
  ...
  // the same color.
  if (type_of_piece(pc) == KING) return AMBIGUITY_NONE;
  ...
  j = 0;
  for (i = 0; i < n; i++)
    if (square_file(move_from(moveList[i])) == square_file(from)) j++;
  if (j == 1) return AMBIGUITY_FILE;

  j = 0;
  for (i = 0; i < n; i++)
    if (square_rank(move_from(moveList[i])) == square_rank(from)) j++;
  if (j == 1) return AMBIGUITY_RANK;
...
const std::string time_string(int milliseconds) {
  ...
  int seconds =
      (milliseconds - hours * 1000 * 60 * 60 - minutes * 60 * 1000) / 1000;
