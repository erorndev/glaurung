...
const Bitboard InFrontBB[2][8] = {
    {Rank2BB | Rank3BB | Rank4BB | Rank5BB | Rank6BB | Rank7BB | Rank8BB,
     ...
     Rank5BB | Rank6BB | Rank7BB | Rank8BB, Rank6BB | Rank7BB | Rank8BB,
     Rank7BB | Rank8BB, Rank8BB, EmptyBoardBB},
    {EmptyBoardBB, Rank1BB, Rank2BB | Rank1BB, Rank3BB | Rank2BB | Rank1BB,
...
const int RShift[64] = {20, 21, 21, 21, 21, 21, 21, 20, 21, 22, 22, 22, 22,
                        22, 22, 21, 21, 22, 22, 22, 22, 22, 22, 21, 21, 22,
                        22, 22, 22, 22, 22, 21, 21, 22, 22, 22, 22, 22, 22,
                        21, 21, 22, 22, 22, 22, 22, 22, 21, 21, 22, 22, 22,
                        22, 22, 22, 21, 20, 21, 21, 21, 21, 21, 21, 20};
...
const int RShift[64] = {52, 53, 53, 53, 53, 53, 53, 52, 53, 54, 54, 54, 54,
...
const int BShift[64] = {26, 27, 27, 27, 27, 27, 27, 26, 27, 27, 27, 27, 27,
                        27, 27, 27, 27, 27, 25, 25, 25, 25, 27, 27, 27, 27,
                        25, 23, 23, 25, 27, 27, 27, 27, 25, 23, 23, 25, 27,
                        27, 27, 27, 25, 25, 25, 25, 27, 27, 27, 27, 27, 27,
                        27, 27, 27, 27, 26, 27, 27, 27, 27, 27, 27, 26};
...
const int BShift[64] = {58, 59, 59, 59, 59, 59, 59, 58, 59, 59, 59, 59, 59,
...
void init_sliding_attacks(Bitboard attacks[], int attackIndex[],
                          Bitboard mask[], const int shift[2],
...
Bitboard sliding_attacks(int sq, Bitboard block, int dirs, int deltas[][2],
                         int fmin = 0, int fmax = 7, int rmin = 0,
                         int rmax = 7) {
  Bitboard result = 0ULL;
  int rk = sq / 8, fl = sq % 8, r, f, i;
  for (i = 0; i < dirs; i++) {
    int dx = deltas[i][0], dy = deltas[i][1];
    for (f = fl + dx, r = rk + dy; (dx == 0 || (f >= fmin && f <= fmax)) &&
                                   (dy == 0 || (r >= rmin && r <= rmax));
         f += dx, r += dy) {
      result |= (1ULL << (f + r * 8));
...
void init_between_bitboards() {
  SquareDelta step[8] = {DELTA_E,  DELTA_W,  DELTA_N,  DELTA_S,
...
void init_sliding_attacks(Bitboard attacks[], int attackIndex[],
  ...
  for (i = 0; i < 64; i++) {
    ...
    for (k = 0; k < j; k++) {
      ...
      attacks[index + (unsigned(int(b) * int(mult[i]) ^
                                int(b >> 32) * int(mult[i] >> 32)) >>
                       shift[i])] = sliding_attacks(i, b, 4, deltas);
#else
      b = index_to_bitboard(k, mask[i]);
  ...
  for (i = 0; i < 64; i++) {
    ...
    for (m = 0; m <= 1; m++) {
      ...
      for (j = 0; j < 8; j++) {
        ...
        if (m)
          FileAttacks[j][(b1 * 0xd6e8802041d0c441ULL) >> 58] = b2;
        else
          RankAttacks[j][i] = b2;
      }
    }
