...
inline Bitboard Position::pieces_of_color_and_type(Color c,
                                                   PieceType pt) const {
...
inline Phase Position::game_phase() const {
  ...
  static const Value MidgameLimit = 2 * QueenValueMidgame +
                                    2 * RookValueMidgame +
                                    6 * BishopValueMidgame + Value(325);
...
inline bool Position::move_is_pawn_push_to_7th(Move m) const {
  ...
  return this->piece_on(move_from(m)) == pawn_of_color(c) &&
...
inline bool Position::move_was_passed_pawn_push(Move m) const {
  ...
  return this->piece_on(move_to(m)) == pawn_of_color(c) &&
         this->pawn_is_passed(c, move_to(m));
}
...
inline bool Position::opposite_colored_bishops() const {
  return this->bishop_count(WHITE) == 1 && this->bishop_count(BLACK) == 1 &&
