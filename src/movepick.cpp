...
MovePicker::MovePicker(Position &p, bool pvnode, Move ttm, Move mk, Move k1,
                       Move k2, Depth dpth) {
  pos = &p;
  pvNode = pvnode;
  ttMove = ttm;
...
Move MovePicker::pick_move_from_list() {
  ...
  case PH_QCHECKS:
    ...
    while (movesPicked < numOfMoves) {
      move = moves[movesPicked++].move;
      // Remember to change the line below if we decide to hash the qsearch!
      if (/* move != ttMove && */ pos->move_is_legal(move, pinned)) return move;
    }
    break;
