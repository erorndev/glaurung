...
void generate_kpk_bitbase(uint8_t bitbase[]) {
  ...
  for (i = 0; i < 24576; i++) {
    for (b = 0, j = 0; j < 8;
         b |= (compress_result(Bitbase[8 * i + j]) << j), j++)
      ;
    bitbase[i] = b;
  }
...
bool KPKPosition::is_legal() const {
  if (whiteKingSquare == pawnSquare || whiteKingSquare == blackKingSquare ||
    ...
    return false;
  if (sideToMove == WHITE) {
    if (bit_is_set(this->wk_attacks(), blackKingSquare)) return false;
    if (bit_is_set(this->pawn_attacks(), blackKingSquare)) return false;
  } else {
    if (bit_is_set(this->bk_attacks(), whiteKingSquare)) return false;
  }
  return true;
}
...
bool KPKPosition::is_immediate_draw() const {
  if (sideToMove == BLACK) {
    ...
    // Case 1: Stalemate
    if ((bka & ~(wka | this->pawn_attacks())) == EmptyBoardBB) return true;
...
bool KPKPosition::is_immediate_win() const {
  ...
  return sideToMove == WHITE && square_rank(pawnSquare) == RANK_7 &&
...
Result classify_wtm(const KPKPosition &p) {
  ...
  while (b) {
    s = pop_1st_bit(&b);
    switch (Bitbase[compute_index(s, p.blackKingSquare, p.pawnSquare, BLACK)]) {
    case RESULT_LOSS:
      return RESULT_WIN;
  ...
  if (square_rank(p.pawnSquare) < RANK_7) {
    ...
    if (square_rank(s) == RANK_3 && s != p.whiteKingSquare &&
        s != p.blackKingSquare) {
      s += DELTA_N;
      switch (Bitbase[compute_index(p.whiteKingSquare, p.blackKingSquare, s,
                                    BLACK)]) {
...
Result classify_btm(const KPKPosition &p) {
  ...
  while (b) {
    s = pop_1st_bit(&b);
    switch (Bitbase[compute_index(p.whiteKingSquare, s, p.pawnSquare, WHITE)]) {
    case RESULT_DRAW:
      return RESULT_DRAW;
