...
inline Piece operator+(Piece p, int i) {
  return Piece(int(p) + i);
}
...
inline PieceType operator+(PieceType p, int i) {
  return PieceType(int(p) + i);
}
inline void operator++(PieceType &p, int) {
  p = PieceType(int(p) + 1);
}
inline PieceType operator-(PieceType p, int i) {
  return PieceType(int(p) - i);
