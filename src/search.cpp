...
// log it for the sexbox will ya
extern "C" {
long long last_search_nodes = 0;
...
Value search(Position &pos, SearchStack ss[], Value beta, Depth depth, int ply,
...
bool split(const Position &pos, SearchStack *ss, int ply, Value *alpha,
...
void think(const Position &pos, bool infinite, bool ponder, int time,
  ...
  if (button_was_pressed("Clear Hash")) TT.clear();
  ...
  SingleReplyExtension[1] =
      Depth(get_option_value_int("Single Reply Extension (PV nodes)"));
  ...
  MaxDepth = maxDepth;
  if (MaxDepth) InfiniteSearch = true; // HACK
...
#ifdef NXDK
  NodesBetweenPolls = 8191; // poll more frequently on sexbox
#endif
...
void id_loop(const Position &pos, Move searchMoves[]) {
  ...
  while (!AbortSearch && Iteration < PLY_MAX) {
    ...
    if (Iteration <= 5) ExtraSearchTime = 0;
    ...
    if (ss[0].pv[0] != EasyMove) EasyMove = MOVE_NONE;
    ...
    if (!InfiniteSearch) {
      ...
      if (Iteration >= 6 && rml.move_count() == 1) stopSearch = true;
      ...
      if (current_search_time() >
          ((MaxSearchTime + ExtraSearchTime) * 80) / 128)
        stopSearch = true;
    ...
    if (MaxDepth && Iteration >= MaxDepth) break;
  }
  ...
  if (PonderSearch)
    ...
    std::cout << "info nodes " << nodes_searched() << " nps " << nps()
              << " time " << current_search_time() << " hashfull " << TT.full()
              << std::endl;

  // Print the best move and the ponder move to the standard output:
  std::cout << "bestmove " << ss[0].pv[0];
  if (ss[0].pv[1] != MOVE_NONE) std::cout << " ponder " << ss[0].pv[1];
  std::cout << std::endl;
  ...
  if (rml.move_count() > 0) {
    searchMoves[0] = rml.get_move(0);
  } else {
    searchMoves[0] = MOVE_NONE;
  }

  last_search_nodes = nodes_searched();
  last_search_time = GetTickCount() - startTime;
}
...
Value root_search(Position &pos, SearchStack ss[], RootMoveList &rml) {
  ...
  for (int i = 0; i < rml.move_count() && !AbortSearch; i++) {
    ...
    if (current_search_time() >= 1000)
      std::cout << "info currmove " << move << " currmovenumber " << i + 1
                << std::endl;
    ...
    if (i < MultiPV) {
      ...
      if (Problem && StopOnPonderhit) StopOnPonderhit = false;
    } else {
    ...
    if (value <= alpha && i >= MultiPV)
      ...
      if (MultiPV == 1) {
        ...
        if (i > 0) BestMoveChangesByIteration[Iteration]++;

        // Print search information to the standard output:
        std::cout << "info depth " << Iteration << " score "
                  << value_to_string(value) << " time " << current_search_time()
                  << " nodes " << nodes_searched() << " nps " << nps()
                  << " pv ";
        ...
        for (int j = 0; j < Min(MultiPV, rml.move_count()); j++) {
          int k;
          std::cout << "info multipv " << j + 1 << " score "
                    << value_to_string(rml.get_move_score(j)) << " depth "
                    << ((j <= i) ? Iteration : Iteration - 1) << " time "
                    << current_search_time() << " nodes " << nodes_searched()
                    << " nps " << nps() << " pv ";
...
Value search_pv(Position &pos, SearchStack ss[], Value alpha, Value beta,
  ...
  if (AbortSearch || thread_should_stop(threadID)) return Value(0);
  ...
  if (ply >= PLY_MAX - 1) return evaluate(pos, ei, threadID);
  ...
  while (alpha < beta && !thread_should_stop(threadID) &&
    ...
    ss[ply].currentMoveCaptureValue =
        move_is_ep(move) ? PawnValueMidgame
    ...
    if (moveCount == 1)
      ...
      if (depth >= 2 * OnePly && ext == Depth(0) && moveCount >= LMRPVMoves &&
          ...
          !move_is_castle(move) && move != ss[ply].killer1 &&
          move != ss[ply].killer2) {
      ...
      if (value > alpha) {
        ...
        if (value > alpha && value < beta) {
          ...
          value =
              -search_pv(pos, ss, -beta, -alpha, newDepth, ply + 1, threadID);
    ...
    if (value > bestValue) {
      ...
      if (value > alpha) {
        alpha = value;
        update_pv(ss, ply);
        if (value == value_mate_in(ply + 1)) ss[ply].mateKiller = move;
      }
    ...
    if (ActiveThreads > 1 && bestValue < beta && depth >= MinimumSplitDepth &&
        Iteration <= 99 && idle_thread_exists(threadID) && !AbortSearch &&
        !thread_should_stop(threadID) &&
        split(pos, ss, ply, &alpha, &beta, &bestValue, depth, &moveCount, &mp,
              dcCandidates, threadID, true))
      break;
  }
...
Value search(Position &pos, SearchStack ss[], Value beta, Depth depth, int ply,
  ...
  // Mate distance pruning
  if (value_mated_in(ply) >= beta) return beta;
  if (value_mate_in(ply + 1) < beta) return beta - 1;
  ...
  if (ttFound) {
    ...
    if (ttDepth >= depth || ttValue >= Max(value_mate_in(100), beta) ||
  ...
  if (!pos.is_check() && allowNullmove && ok_to_do_nullmove(pos) &&
    ...
    pos.do_null_move(u);
    nullValue = -search(pos, ss, -(beta - 1), depth - 4 * OnePly, ply + 1,
                        false, threadID);
    ...
    if (nullValue >= beta) {
      if (depth >= 6 * OnePly) { // Do zugzwang verification search
        Value v =
            search(pos, ss, beta, depth - 5 * OnePly, ply, false, threadID);
      ...
      if (nullValue == value_mated_in(ply + 2)) mateThreat = true;
  ...
  if (UseIIDAtNonPVNodes && ttMove == MOVE_NONE && depth >= 8 * OnePly &&
    ...
    search(pos, ss, beta, Min(depth / 2, depth - 2 * OnePly), ply, false,
           threadID);
    ttMove = ss[ply].pv[ply];
  }
  ...
  while (bestValue < beta && !thread_should_stop(threadID) &&
    ...
    if (useFutilityPruning && ext == Depth(0) && !moveIsCapture &&
      ...
      if (depth < 6 * OnePly && approximateEval < beta) {
        ...
        if (futilityValue < beta) {
          if (futilityValue > bestValue) bestValue = futilityValue;
          continue;
        }
      }
    }
    ...
    if (value > bestValue) {
      bestValue = value;
      if (value >= beta) update_pv(ss, ply);
      if (value == value_mate_in(ply + 1)) ss[ply].mateKiller = move;
    }
    ...
    if (ActiveThreads > 1 && bestValue < beta && depth >= MinimumSplitDepth &&
        Iteration <= 99 && idle_thread_exists(threadID) && !AbortSearch &&
        !thread_should_stop(threadID) &&
        split(pos, ss, ply, &beta, &beta, &bestValue, depth, &moveCount, &mp,
              dcCandidates, threadID, false))
      break;
  }
...
Value qsearch(Position &pos, SearchStack ss[], Value alpha, Value beta,
  ...
  MovePicker mp =
      MovePicker(pos, false, MOVE_NONE, MOVE_NONE, MOVE_NONE, MOVE_NONE, depth);
  Move move;
  int moveCount = 0;
  Bitboard dcCandidates = mp.discovered_check_candidates();
  bool isCheck = pos.is_check();
  ...
  while (alpha < beta && ((move = mp.get_next_move()) != MOVE_NONE)) {
    ...
    if (UseQSearchFutilityPruning && !isCheck && !moveIsCheck &&
        !move_promotion(move) && !moveIsPassedPawnPush && beta - alpha == 1 &&
        pos.non_pawn_material(pos.side_to_move()) > RookValueMidgame) {
      Value futilityValue = staticValue +
                            Max(pos.midgame_value_of_piece_on(move_to(move)),
                                pos.endgame_value_of_piece_on(move_to(move))) +
                            FutilityMargin0 + ei.futilityMargin;
...
void sp_search(SplitPoint *sp, int threadID) {
  ...
  while (sp->bestValue < sp->beta && !thread_should_stop(threadID) &&
    ...
    if (ext == Depth(0) && moveCount >= LMRNonPVMoves && !moveIsCapture &&
        !move_promotion(move) && !moveIsPassedPawnPush &&
        !move_is_castle(move) && move != ss[sp->ply].killer1 &&
    ...
    if (thread_should_stop(threadID)) break;
...
void sp_search_pv(SplitPoint *sp, int threadID) {
  ...
  while (sp->alpha < sp->beta && !thread_should_stop(threadID) &&
    ...
    if (ext == Depth(0) && moveCount >= LMRPVMoves && !moveIsCapture &&
        ...
        move != ss[sp->ply].killer2) {
      ss[sp->ply].reduction = OnePly;
      value = -search(pos, ss, -sp->alpha, newDepth - OnePly, sp->ply + 1, true,
                      threadID);
    } else
      value = sp->alpha + 1;
    if (value > sp->alpha) {
      ss[sp->ply].reduction = Depth(0);
      value =
          -search(pos, ss, -sp->alpha, newDepth, sp->ply + 1, true, threadID);
    ...
    if (value > sp->bestValue && !thread_should_stop(threadID)) {
      ...
      if (value > sp->alpha) {
        sp->alpha = value;
        sp_update_pv(sp->parentSstack, ss, sp->ply);
        if (value == value_mate_in(sp->ply + 1)) ss[sp->ply].mateKiller = move;
        if (value >= sp->beta) {
          for (int i = 0; i < ActiveThreads; i++)
            if (i != threadID && (i == sp->master || sp->slaves[i]))
              Threads[i].stop = true;
...
RootMoveList::RootMoveList(Position &pos, Move searchMoves[]) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (includeMove) {
      ...
      moves[j].score =
          -qsearch(pos, ss, -VALUE_INFINITE, VALUE_INFINITE, Depth(0), 1, 0);
...
int RootMoveList::compare_root_moves(const RootMove &rm1, const RootMove &rm2) {
  if (rm1.score < rm2.score)
    return 1;
  else if (rm1.score > rm2.score)
    return 0;
...
void init_node(const Position &pos, SearchStack ss[], int ply, int threadID) {
  ...
  if (Threads[threadID].printCurrentLine) print_current_line(ss, ply, threadID);
}
...
bool connected_moves(const Position &pos, Move m1, Move m2) {
  ...
  // in m1:
  if (pos.piece_attacks_square(t1, t2)) return true;
  ...
  if (piece_is_slider(pos.piece_on(t1)) &&
    ...
    if (pos.type_of_piece_on(t1) == BISHOP) {
      ...
      if (bit_is_set(rook_attacks_bb(ksq, occ), t1)) return true;
    } else {
      assert(pos.type_of_piece_on(t1) == QUEEN);
      if (bit_is_set(queen_attacks_bb(ksq, occ), t1)) return true;
    }
  }
...
Depth extension(const Position &pos, Move m, bool pvNode, bool check,
  ...
  if (check) result += CheckExtension[pvNode];
  if (singleReply) result += SingleReplyExtension[pvNode];
  if (pos.move_is_pawn_push_to_7th(m)) result += PawnPushTo7thExtension[pvNode];
  if (pos.move_is_passed_pawn_push(m)) result += PassedPawnExtension[pvNode];
  if (mateThreat) result += MateThreatExtension[pvNode];
...
bool ok_to_do_nullmove(const Position &pos) {
  if (pos.non_pawn_material(pos.side_to_move()) == Value(0)) return false;
  return true;
}
...
bool ok_to_prune(const Position &pos, Move m, Move threat, Depth d) {
  ...
  if (move_is_castle(m)) return false;

  // Case 2: Don't prune moves which move the threatened piece
  if (!PruneEscapeMoves && threat != MOVE_NONE && mfrom == tto) return false;
  ...
  if (!H.ok_to_prune(pos.piece_on(move_from(m)), m, d)) return false;
...
bool fail_high_ply_1() {
  for (int i = 0; i < ActiveThreads; i++)
    if (Threads[i].failHighPly1) return true;
  return false;
}
...
void poll() {
  ...
  if (data) {
    char input[256];
    if (fgets(input, 255, stdin) == NULL) strcpy(input, "quit\n");
  ...
  if (t < 1000)
    ...
    lock_release(&IOLock);
    if (ShowCurrentLine) Threads[0].printCurrentLine = true;
  }
  ...
  if (!PonderSearch && ExactMaxTime && t >= ExactMaxTime) AbortSearch = true;
...
void ponderhit() {
  ...
  if (Iteration >= 2 &&
      (!InfiniteSearch &&
       (StopOnPonderhit || t > AbsoluteMaxSearchTime ||
        (RootMoveNumber == 1 && t > MaxSearchTime + ExtraSearchTime) ||
        (!FailHigh && !fail_high_ply_1() && !Problem &&
         t > 6 * (MaxSearchTime + ExtraSearchTime)))))
    AbortSearch = true;
...
void idle_loop(int threadID, SplitPoint *waitSp) {
  ...
  while (true) {
    ...
    if (waitSp != NULL && waitSp->cpus == 0) return;
  }
...
bool thread_should_stop(int threadID) {
  ...
  if (Threads[threadID].stop) return true;
  if (ActiveThreads <= 2) return false;
...
bool thread_is_available(int slave, int master) {
  ...
  if (!Threads[slave].idle || slave == master) return false;
  ...
  if (ActiveThreads == 2) return true;

  // Apply the "helpful master" concept if possible.
  if (SplitPointStack[slave][Threads[slave].activeSplitPoints - 1]
          .slaves[master])
    return true;
...
bool idle_thread_exists(int master) {
  ...
  for (int i = 0; i < ActiveThreads; i++)
    if (thread_is_available(i, master)) return true;
  return false;
}
...
bool split(const Position &p, SearchStack *sstck, int ply, Value *alpha,
           Value *beta, Value *bestValue, Depth depth, int *moves,
           MovePicker *mp, Bitboard dcCandidates, int master, bool pvNode) {
  assert(p.is_ok());
  assert(sstck != NULL);
